...
// @route   GET api/posts
// @desc    get all posts
// @access  Private - have to be logged in to see posts
router.get('/', auth, async (req, res) => {
    try{
        const posts = await Post.find().sort({ date: -1 }) //sorts in ascending order
        res.json(posts)
    } catch(err){
        console.error(err.message)
        res.status(500).send('Server error')
...
router.delete('/:id', auth, async (req, res) => {
    try{
        ...
        //check if current user = post user
...
router.delete('/comment/:id/:comment_id', auth, async (req, res) => {
    try{
        ...
        //make sure user deleteing comment is user who wrote comment
